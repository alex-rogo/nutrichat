...
export default function ChatSection({ onConfirmMeal, onDeleteMeal, onSaveMeal, initialMeals, onOpenInsights, externalRecipe, onClearRecipe }: Props) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([{ id: 'welcome', isUser: false, type: 'text', text: "Hi! Tell me what you ate..." }]);
  ...
  useEffect(() => {
    if (externalRecipe) {
      setMessages([
        { id: 'welcome', isUser: false, type: 'text', text: "Hi! Tell me what you ate..." },
        { id: 'recipe-' + Date.now(), isUser: false, type: 'recipe', recipe: externalRecipe, confirmed: false, _foodName: externalRecipe.name } as any
      ]);
      setChatOpen(true);
  ...
  const handleSend = (text: string) => {
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => [
        ...
        { id: (Date.now() + 1).toString(), isUser: false, type: 'estimate', estimate, confirmed: false, _foodName: text.length > 25 ? text.substring(0, 22) + '...' : text } as any,
      ]);
    }, 1200);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <TouchableOpacity style={[styles.fab, { backgroundColor: theme.primary || '#39ff14', shadowColor: theme.primary || '#39ff14' }]} onPress={openChat}>
          <Text style={styles.fabIcon}>+</Text> 
        </TouchableOpacity>
      </LinearGradient>

      <Modal visible={chatOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setChatOpen(false)}>
        <KeyboardAvoidingView style={[styles.modalContainer, { backgroundColor: theme.mainBg || '#0d0d0d' }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 40 : 0}>
          ...
          <ScrollView ref={scrollRef} style={styles.chatScroll} contentContainerStyle={styles.chatContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            {messages.map((message) => (
              <MessageBubble key={message.id} {...message} onConfirm={message.type === 'estimate' || message.type === 'recipe' ? () => handleConfirm(message.id) : undefined} />
            ))}
          </ScrollView>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  recentHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', paddingHorizontal: 24, marginBottom: 16, marginTop: 10 },
  recentTitle: { fontSize: 24, fontWeight: '800', marginBottom: 2 },
  recentSubtitle: { fontSize: 12 },
  logList: { flex: 1, paddingHorizontal: 24 },
  logListContent: { gap: 16, paddingBottom: 140 },
  ...
  foodInfo: { flex: 1 },
  foodTitle: { fontWeight: '700', fontSize: 15, marginBottom: 4, textTransform: 'capitalize' },
  foodMeta: { fontSize: 11, marginBottom: 8 },
  foodMacros: { flexDirection: 'row', gap: 10 },
  macroBadge: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  ...
  chatHeaderText: { fontWeight: '700', fontSize: 17 },
  closeBtn: { padding: 4 },
  closeBtnText: { fontSize: 20 },
  chatScroll: { flex: 1 },
  chatContent: { padding: 24, gap: 16, paddingBottom: 16 },
  chatInputArea: { paddingHorizontal: 24, paddingTop: 12, paddingBottom: 28, borderTopWidth: 1 },
});